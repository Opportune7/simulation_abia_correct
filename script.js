...
function renderOverview(data){
  ...
  // ── Projects by Cluster bar ──
  const clusterData = CLUSTERS.map(c=>({name:c,budget:sum(data.filter(p=>p.cluster===c),"budget"),count:data.filter(p=>p.cluster===c).length}));
  destroyChart("cluster");
  charts.cluster = new Chart(document.getElementById("ch-cluster"),{
    type:"bar",
    data:{labels:clusterData.map(d=>d.name),datasets:[{label:"Budget",data:clusterData.map(d=>d.budget),backgroundColor:CLUSTERS.map(c=>CLUSTER_COLORS[c]),borderRadius:6}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>fmtN(ctx.raw)}}},scales:{y:{ticks:{callback:v=>fmtN(v)}},x:{ticks:{font:{size:11}}}}}
  });
  ...
  charts.mix = new Chart(document.getElementById("ch-mix"),{
    type:"doughnut",
    data:{labels:clusterData.map(d=>d.name),datasets:[{data:clusterData.map(d=>d.count),backgroundColor:CLUSTERS.map(c=>CLUSTER_COLORS[c]),borderWidth:2}]},
  ...
  // ── Progress by Priority bars ──
  const priHTML = ["Priority","Non-priority"].map(pri=>{
    ...
    return `<div style="margin-bottom:20px">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px">
        <span style="font-weight:500">${pri}</span>
        <span style="font-weight:700">${avg}% <span style="font-weight:400;font-size:11px;color:var(--muted)">(${items.length} projects)</span></span>
      </div>
      <div style="height:10px;background:var(--border);border-radius:5px;overflow:hidden">
        <div style="height:100%;width:${avg}%;background:var(--primary);border-radius:5px"></div>
      </div>
    </div>`;
  }).join("");
  document.getElementById("ov-priority-bars").innerHTML = priHTML;

  // ── Inline SDG treemap ──
  const bySdgInline = groupBy(data,"sdg");
  const treeInline = Object.keys(bySdgInline).map(s=>({sdg:s,value:sum(bySdgInline[s],"budget"),count:bySdgInline[s].length}));
  destroyChart("sdg-inline");
  charts["sdg-inline"] = new Chart(document.getElementById("ch-sdg-inline"),{
    type:"treemap",
    data:{datasets:[{tree:treeInline,key:"value",
      backgroundColor:ctx=>ctx.raw?SDG_COLORS[ctx.raw._data.sdg]:"#ccc",
      borderColor:"#fff",borderWidth:2,spacing:1,
      labels:{display:true,color:"#fff",font:{weight:"600",size:10},
        formatter:ctx=>ctx.raw._data?[ctx.raw._data.sdg]:[]}}]},
